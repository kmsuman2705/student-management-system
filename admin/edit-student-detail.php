...
if (strlen($_SESSION['sturecmsaid']==0)) {
 ...
 $tecqlf = $_POST['tecqlf'];
 $eid=$_GET['editid'];
$sql="update tblstudent set StudentName=:stuname,StudentEmail=:stuemail,StudentClass=:stuclass,Gender=:gender,DOB=:dob,RegNo=:regno,FatherName=:fname,MotherName=:mname,ContactNumber=:connum,AadharNumber=:aadharnum,State=:state,AcademicQlf=:acdqlf,TechnicalQlf=:tecqlf where ID=:eid";
...
$query->bindParam(':aadharnum',$aadharnum,PDO::PARAM_STR);
$query->bindParam(':state',$state,PDO::PARAM_STR);
$query->bindParam(':acdqlf',$acdqlf,PDO::PARAM_STR);
$query->bindParam(':tecqlf',$tecqlf,PDO::PARAM_STR);
                ...
                <div class="card">
                  <div class="card-body">
                    ...
                    <form class="forms-sample" method="post" enctype="multipart/form-data">
                      <?php
$eid=$_GET['editid'];
$sql="SELECT tblstudent.StudentName,tblstudent.StudentEmail,tblstudent.StudentClass,tblstudent.Gender,tblstudent.DOB,tblstudent.RegNo,tblstudent.FatherName,tblstudent.MotherName,tblstudent.ContactNumber,tblstudent.AadharNumber,tblstudent.State,tblstudent.AcademicQlf,tblstudent.TechnicalQlf,tblstudent.UserName,tblstudent.Password,tblstudent.Image,tblstudent.DateofAdmission,tblclass.SchemeName,tblclass.TradeName from tblstudent join tblclass on tblclass.ID=tblstudent.StudentClass where tblstudent.ID=:eid";
$query = $dbh -> prepare($sql);
$query->bindParam(':eid',$eid,PDO::PARAM_STR);
$query->execute();
...
{               ?>
                      ...
                      <div class="form-group">
                        ...
                        <select  name="stuclass" class="form-control" required='true'>
                          <option value="<?php  echo htmlentities($row->StudentClass);?>"><?php  echo htmlentities($row->SchemeName);?> <?php  echo htmlentities($row->TradeName);?></option>
                         <?php 
...
{          
    ?>  
<option value="<?php echo htmlentities($row1->SchemeName);?><?php echo htmlentities($row1->TradeName);?>"><?php echo htmlentities($row1->SchemeName);?> <?php echo htmlentities($row1->TradeName);?></option>
 <?php } ?> 
                        </select>
                      </div>
                      <div class="form-group">
                      ...
                      <div class="form-group">
                        ...
                        <input type="text" name="regno" value="<?php  echo htmlentities($row->RegNo);?>" class="form-control" readonly='true'>
                      </div>
                      <div class="form-group">
                      ...
                      <div class="form-group">
                        ...
                        <input type="text" name="aadharnum" value="<?php  echo htmlentities($row->AadharNumber);?>" class="form-control" required='true' maxlength="10" pattern="[0-9]+">
                      </div>
                      <div class="form-group">
                        <label for="exampleInputName1">State</label>
                        <textarea name="state" class="form-control" required='true'><?php  echo htmlentities($row->State);?></textarea>
                      </div>
                      <div class="form-group">
                        <label for="exampleInputName1">Academic Qualification</label>
                        <textarea name="acdqlf" class="form-control" required='true'><?php  echo htmlentities($row->AcademicQlf);?></textarea>
                      </div>
                      <div class="form-group">
                        <label for="exampleInputName1">Technical Qualification</label>
                        <textarea name="tecqlf" class="form-control" required='true'><?php  echo htmlentities($row->TechnicalQlf);?></textarea>
                      </div>
<h3>Login details</h3>
<div class="form-group">
